...
const BUNDLE_SCRIPT_PATTERN =
  /`"\$NODE_BINARY"\s+--print\s+"require\('path'\)\.dirname\(require\.resolve\('react-native\/package\.json'\)\)\s\+\s'\/scripts\/react-native-xcode\.sh'"\s*`/;
...
function patchBundleBuildPhase(config) {
  return withXcodeProject(config, (cfg) => {
    ...
    for (const key of Object.keys(phases)) {
      ...
      if (script.includes('RN_XCODE_SCRIPT_PATH=')) continue;
      if (!BUNDLE_SCRIPT_PATTERN.test(script)) continue;

      phase.shellScript = script.replace(BUNDLE_SCRIPT_PATTERN, BUNDLE_SCRIPT_PATCH);
    }
